...
public class GbifAuthenticationManagerTest {
  ...
  @InjectMocks
  private GbifAuthenticationManager manager;
  ...
  public void testBasicSuccess() {
    ...
    GbifAuthentication mockAuthentication = mock(GbifAuthentication.class);
    when(mockAuthentication.getRequest()).thenReturn(mockRequest);
    ...
    // WHEN
    final GbifAuthentication result = ((GbifAuthentication) manager.authenticate(mockAuthentication));
  ...
  public void testBasicFailWrongPassword() {
    ...
    // WHEN & THEN
    try {
      manager.authenticate(mockAuthentication);
  ...
  public void testGbifFailAuthServiceIsUnavailable() {
    ...
    when(mockRequest.getHeader(HttpHeaders.AUTHORIZATION)).thenReturn("GBIF user:blabla");
    when(mockRequest.getHeader(SecurityConstants.HEADER_GBIF_USER)).thenReturn("user");
    GbifAuthenticationManager manager = new GbifAuthenticationManager(identityAccessServiceMock, null);
