...
public class DataCiteConverter {
   ...
   */
  @VisibleForTesting
  public static DataCiteMetadata convertCustomDownload(DOI doi, String size, String numberRecords, String creatorName,
    ...
    DataCiteMetadata.Builder<Void> b = DataCiteMetadata.builder().withIdentifier()
      .withIdentifierType(IdentifierType.DOI.name()).withValue(doi.getDoiName()).end()
      .withTitles()
      ...
      .end()
      .withCreators()
      .addCreator().withCreatorName(creatorName).withNameIdentifier(
        DataCiteConverter.userIdToCreatorNameIdentifier(creatorUserId)).end()
      .end()
      .withPublisher(GBIF_PUBLISHER)
      .withPublicationYear(DataCiteConverter.getYear(created))
      .withResourceType().withResourceTypeGeneral(ResourceType.DATASET).end().withDates().addDate()
      .withDateType(DateType.CREATED).withValue(fdate(created)).end().addDate()
      .withDateType(DateType.UPDATED).withValue(fdate(created)).end()
      .end().withFormats().addFormat(DWCA_FORMAT).end().withSizes().addSize(size).end();
    ...
    if (!usedDatasets.isEmpty()) {
      ...
      db.addContent("Data from some individual datasets included in this download may be licensed under less restrictive terms.");
    }
    return b.build();
  }
