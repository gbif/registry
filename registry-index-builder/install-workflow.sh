...
solr_collection_opts=`cat $P.properties| grep "oozie.url" | cut -d'=' -f2`
...
echo "Delete existing collection"
curl -s """${solr_url}"/admin/collections?action=CREATE\&name="${solr_collection}"\&"${solr_collection_opts}"\&collection.configName="${solr_collection}"""
