...
public class OaipmhGetRecordIT extends AbstractOaipmhEndpointIT {
  ...
  public void getRecordWithAugmentedMetadata() throws Exception {
    ...
    String result = IOUtils.toString(new URI(baseUrl + "?verb=GetRecord&metadataPrefix=" + EML_FORMAT.getMetadataPrefix() +
            "&identifier=" + record.getHeader().getIdentifier().toString()).toURL(), "UTF-8");

    // ensure we get the augmented metadata data
    assertTrue("GetRecord verb returns augmented metadata", result.contains("<citation identifier=\"doi:tims-ident.2136.ex43.33.d\">title 1</citation>"));
  }
