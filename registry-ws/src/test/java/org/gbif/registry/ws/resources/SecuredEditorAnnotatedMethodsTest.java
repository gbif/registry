...
public class SecuredEditorAnnotatedMethodsTest {
  ...
  private static final List<Pattern> DATASET_RESOURCE_WITH_KEY =
      Arrays.asList(
          ...
          Pattern.compile(
              "^/dataset/[0-9a-f-]+/(comment|tag|contact|endpoint|identifier)/[0-9]+$"));

  private static final Pattern INSTALLATION_RESOURCE_WITHOUT_KEY =
      Pattern.compile("^/installation$");
  private static final List<Pattern> INSTALLATION_RESOURCE_WITH_KEY =
      Arrays.asList(
          Pattern.compile("^/installation/[0-9a-f-]+$"),
          Pattern.compile("^/installation/[0-9a-f-]+/(comment|tag|contact|endpoint|identifier)$"),
          Pattern.compile(
              "^/installation/[0-9a-f-]+/(comment|tag|contact|endpoint|identifier)/[0-9]+$"));

  private static final Pattern ORGANIZATION_RESOURCE_WITHOUT_KEY =
      Pattern.compile("^/organization$");
  private static final List<Pattern> ORGANIZATION_RESOURCE_WITH_KEY =
      Arrays.asList(
          Pattern.compile("^/organization/[0-9a-f-]+$"),
          Pattern.compile(
              "^/organization/[0-9a-f-]+/(comment|tag|contact|endpoint|identifier|password|endorsement)$"),
          Pattern.compile(
              "^/organization/[0-9a-f-]+/(comment|tag|contact|endpoint|identifier)/[0-9]+$"));

  private static final Pattern NODE_NETWORK_RESOURCE_WITHOUT_KEY =
      Pattern.compile("^/(node|network)$");

  private static final List<Pattern> NODE_NETWORK_RESOURCE_WITH_KEY =
      Arrays.asList(
          Pattern.compile("^/(node|network)/[0-9a-f-]+$"),
          Pattern.compile("^/(node|network)/[0-9a-f-]+/(comment|tag|contact|endpoint|identifier)$"),
          Pattern.compile(
              "^/(node|network)/[0-9a-f-]+/(comment|tag|contact|endpoint|identifier|constituents)/[a-f0-9-]+$"));

  private static final List<Pattern> PIPELINES_RESOURCE =
      Arrays.asList(
          ...
          if (containsEditor) {
            ...
            Set<String> fullPaths =
                Arrays.stream(requestMapping.value())
                    ...
                    .flatMap(
                        item ->
                            Arrays.stream(getMethodPaths(method))
                                .map(SecuredEditorAnnotatedMethodsTest::prependWithSlash)
                                .map(item::concat))
  ...
  private void mockSpecific(String requestPath, boolean isAllowedToModify) throws Exception {
    if (DATASET_RESOURCE_WITHOUT_KEY.matcher(requestPath).matches()) {
      when(mockEditorAuthService.allowedToModifyDataset(USERNAME, DATASET))
      ...
      when(mockEditorAuthService.allowedToModifyInstallation(USERNAME, INSTALLATION))
          .thenReturn(isAllowedToModify);
      when(objectMapper.readValue(CONTENT, Installation.class)).thenReturn(INSTALLATION);
      when(mockRequest.getContent()).thenReturn(CONTENT);
    } else if (INSTALLATION_RESOURCE_WITH_KEY.stream()
      ...
      when(mockEditorAuthService.allowedToModifyOrganization(USERNAME, ORG))
          .thenReturn(isAllowedToModify);
      when(objectMapper.readValue(CONTENT, Organization.class)).thenReturn(ORG);
      when(mockRequest.getContent()).thenReturn(CONTENT);
    } else if (ORGANIZATION_RESOURCE_WITH_KEY.stream()
        .anyMatch(p -> p.matcher(requestPath).matches())) {
      when(mockEditorAuthService.allowedToModifyOrganization(USERNAME, KEY))
          .thenReturn(isAllowedToModify);
    } else if (NODE_NETWORK_RESOURCE_WITH_KEY.stream()
        .anyMatch(p -> p.matcher(requestPath).matches())) {
      ...
      when(mockEditorAuthService.allowedToModifyDataset(USERNAME, SUB_DATASET_KEY))
          .thenReturn(isAllowedToModify);
    } else if (MACHINE_TAG_RESOURCE_WITHOUT_KEY.matcher(requestPath).matches()) {
      when(mockEditorAuthService.allowedToCreateMachineTag(USERNAME, KEY, MACHINE_TAG))
          .thenReturn(isAllowedToModify);
