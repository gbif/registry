...
.controller('DatasetCtrl', function ($scope, $state, $stateParams, notifications, Restangular, DEFAULT_PAGE_SIZE) {
  ...
  $scope.redirectTo = function (type, key) {
     ...
     * dataset is owned by a non-endorsed organization, since such a dataset should only ever be crawled in testing.
     * @param isEndorsed whether the owning organization of the dataset is endorsed or not
     */
    $scope.generateCrawlerWarningMsg = function (isEndorsed) {
      ...
      (isEndorsed.toString().toLowerCase() == "true") ? "This will trigger a crawl of the dataset. "
        : "This dataset\'s owning organization is not endorsed yet! This will trigger a crawl of the dataset, and should only be done in a test environment";
    }
